...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['username'])) {
        ...
        $stmt = $conn->prepare("UPDATE admins SET status = 'blocked' WHERE username = ?");
        if ($stmt) {
            $stmt->bind_param("s", $username);
            if ($stmt->execute()) {
                header("Location: admin.php?message=User '$username' has been blocked.");
                exit();
            } else {
                // Handle execution error
                header("Location: admin.php?error=Failed to block user.");
                exit();
            }
        } else {
